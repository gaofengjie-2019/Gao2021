...
def plot_heatmaps(A, B, taxon_row_names, taxon_col_names, antibiotic_row_names, antibiotic_col_names, suffix, which_plot="A"):
    ...
    if which_plot == "A":
        ...
        m = np.abs(A).max()
        ax = sns.heatmap(A, xticklabels=taxon_col_names, annot=False, annot_kws={"size": 7}, yticklabels=taxon_row_names, center = 0, vmin=-m, vmax=m, cmap="coolwarm")
    ...
    if which_plot == "B":
        ...
        m = np.abs(B).max()
        sns.heatmap(B, ax=ax, xticklabels=antibiotic_col_names, annot=False, annot_kws={"size": 7}, yticklabels=antibiotic_row_names, center = 0, vmin=-m, vmax=m, cmap="coolwarm")
        plt.setp(ax.get_xticklabels(),fontsize=8)
        plt.setp(ax.get_yticklabels(),fontsize=8)
        plt.tight_layout()
        plt.tight_layout()
...
if __name__ == "__main__":
    ...
    for y in Y:
        P.append(y / y.sum(axis=1,keepdims=True))#求相对丰度
    ...
    plot_heatmaps(A, B, taxon_row_names, col_names, antibiotics, antibiotics, "clv-A1", "A")

    perturb_denom = choose_perturb_denom(P)
    A_new, g_new, B_new, antibiotic_row_names = swap_denom(A, g, B, perturb_denom, col_names)
    plot_heatmaps(A_new, B_new, taxon_row_names, col_names, antibiotic_row_names, antibiotics, "clv-B1", "B")
